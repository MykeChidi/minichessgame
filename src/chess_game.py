...
class Controller:
        ...
        if self.game_state == GameState.PUZZLE and not self.puzzle_failed:
            if not self.ai.board.is_game_over() and len(self.ai.board.move_stack) > 0:
                ...
                if undone_move:
                    self.last_move = (
                        self.ai.board.peek()
                        if len(self.ai.board.move_stack) > 0
                        else None
                    )
                    self.selected_square = None
                    self.legal_moves = []
    ...
    def validate_game_state_transition(self, new_state):
        ...
        valid_transitions = {
            GameState.MENU: [
                GameState.TRAINING,
                GameState.HUMAN_VS_AI,
                GameState.HUMAN_VS_HUMAN,
                GameState.PUZZLE,
            ],
        ...
        def training_worker():
            ...
            for game_num in range(num_games):
                ...
                if game_num % 25 == 0 and game_num > 0:
                    self.ai.epsilon = max(
                        self.ai.epsilon_min, self.ai.epsilon * self.ai.epsilon_decay
                    )
    ...
    def _handle_pawn_promotion(self, move, square):
        ...
        if (
            ...
            and self.ai.board.piece_at(self.selected_square).piece_type == chess.PAWN
        ):
            # Check if pawn is reaching promotion rank
            if (
                self.ai.board.turn == chess.WHITE and chess.square_rank(square) == 7
            ) or (self.ai.board.turn == chess.BLACK and chess.square_rank(square) == 0):
                # Randomly choose between Queen and Rook
                promotion_piece = random.choice([chess.QUEEN, chess.ROOK])
                move = chess.Move(
                    move.from_square, move.to_square, promotion=promotion_piece
                )
    ...
    def handle_square_click(self, square: int):
        ...
        if self.game_state == GameState.PUZZLE:
            ...
            if piece and piece.color == self.player_color:
                ...
                self.legal_moves = [
                    m for m in self.ai.board.legal_moves if m.from_square == square
                ]
            elif self.selected_square is not None:
                move = next(
                    (m for m in self.legal_moves if m.to_square == square), None
                )
                if move:
                    ...
                    if self.ai.board.is_checkmate():
                        ...
                        self.show_message("Puzzle Failed. Out of moves.", 5000)
        ...
        if self.game_state == GameState.HUMAN_VS_HUMAN:
            ...
            if piece and piece.color == self.ai.board.turn:
                ...
                self.legal_moves = [
                    move
                    for move in self.ai.board.legal_moves
        ...
        if piece and piece.color == self.player_color:
            self.selected_square = square
            self.legal_moves = [
                move for move in self.ai.board.legal_moves if move.from_square == square
            ]
...
class RenderUI:
    ...
    def _draw_piece_images(self):
        ...
        for piece_type in ["P", "R", "N", "B", "Q", "K", "p", "r", "n", "b", "q", "k"]:
            ...
            if piece_type.upper() == "P":  # Pawn
                # Draw pawn shape
                pygame.draw.circle(
                    piece_surface, outline_color, (center_x, center_y - 8), 12
                )
                pygame.draw.circle(
                    piece_surface, piece_color, (center_x, center_y - 8), 10
                )
                pygame.draw.rect(
                    piece_surface, outline_color, (center_x - 8, center_y + 2, 16, 12)
                )
                pygame.draw.rect(
                    piece_surface, piece_color, (center_x - 6, center_y + 4, 12, 8)
                )

            elif piece_type.upper() == "R":  # Rook
                # Draw rook shape
                pygame.draw.rect(
                    piece_surface, outline_color, (center_x - 12, center_y - 8, 24, 20)
                )
                pygame.draw.rect(
                    piece_surface, piece_color, (center_x - 10, center_y - 6, 20, 16)
                )
                # Crenellations
                for i in range(0, 5):
                    x = center_x - 10 + i * 4
                    if i % 2 == 0:
                        pygame.draw.rect(
                            piece_surface, outline_color, (x, center_y - 12, 4, 6)
                        )
                        pygame.draw.rect(
                            piece_surface, piece_color, (x + 1, center_y - 11, 2, 4)
                        )
                ...
                # Inner shape
                inner_points = [
                    (x + (1 if x < center_x else -1), y + 1) for x, y in points[:-1]
                ]
                ...
                # Draw bishop shape
                pygame.draw.circle(
                    piece_surface, outline_color, (center_x, center_y - 8), 8
                )
                pygame.draw.circle(
                    piece_surface, piece_color, (center_x, center_y - 8), 6
                )
                pygame.draw.polygon(
                    piece_surface,
                    outline_color,
                ...
                for i in range(-2, 3):
                    x = center_x + i * 4
                    pygame.draw.circle(
                        piece_surface, outline_color, (x, center_y - 10 - abs(i) * 2), 2
                    )
                ...
                # Crown
                pygame.draw.rect(
                    piece_surface, outline_color, (center_x - 8, center_y - 8, 16, 6)
                )
                pygame.draw.rect(
                    piece_surface, piece_color, (center_x - 6, center_y - 6, 12, 2)
                )
                # Cross on top
                pygame.draw.line(
                    piece_surface,
    ...
    def _load_piece_images_from_files(self):
        ...
        piece_files = {
            "K": "pieces/king_white.png",
            "Q": "pieces/queen_white.png",
            "R": "pieces/rook_white.png",
            "B": "pieces/bishop_white.png",
            "N": "pieces/knight_white.png",
            "P": "pieces/pawn_white.png",
            "k": "pieces/king_black.png",
            "q": "pieces/queen_black.png",
            "r": "pieces/rook_black.png",
            "b": "pieces/bishop_black.png",
            "n": "pieces/knight_black.png",
            "p": "pieces/pawn_black.png",
        }
        ...
        for piece_symbol, filename in piece_files.items():
            try:
                ...
                scaled_image = pygame.transform.scale(
                    image, (SQUARE_SIZE - 4, SQUARE_SIZE - 4)
                )
    ...
    def draw_sidebar(self):
        ...
        pygame.draw.rect(
            self.screen, BG_COLOR, (sidebar_x, 0, SIDEBAR_WIDTH, WINDOW_HEIGHT)
        )
        ...
        if self.game_state == GameState.HUMAN_VS_AI:
            ...
            # Current turn
            current_player = (
                "White" if self.ai.board.turn == self.player_color else "Black"
            )
            if self.ai_thinking:
                current_player = "AI thinking..."

            text = self.font.render(
                f"Current player: {current_player}", True, TEXT_COLOR
            )
            self.screen.blit(text, (sidebar_x + 10, y_offset))
            y_offset += 40
            ...
            if self.controller.current_puzzle:
                ...
                text = self.font.render(
                    f"Objective: Checkmate in {mate_in} move(s)", True, TEXT_COLOR
                )
                self.screen.blit(text, (sidebar_x + 10, y_offset))
                y_offset += 25

                text = self.font.render(
                    f"Moves: {moves_made} / {mate_in}", True, TEXT_COLOR
                )
                self.screen.blit(text, (sidebar_x + 10, y_offset))
                y_offset += 40
            ...
            text = self.font.render(
                f"Games: {progress['games']}/{progress['total']}", True, TEXT_COLOR
            )
            self.screen.blit(text, (sidebar_x + 10, y_offset))
            y_offset += 20

            text = self.font.render(
                f"Wins: {progress['wins']} Draws: {progress['draws']} Losses: {progress['losses']}",
                True,
                TEXT_COLOR,
            )
            self.screen.blit(text, (sidebar_x + 10, y_offset))
        ...
        if self.game_state == GameState.PAUSED:
            buttons = [
                (
                    "Resume Game",
                    lambda: self.controller.set_game_state(GameState.HUMAN_VS_AI),
                ),
            ...
            buttons = [("Stop Training", self.controller.stop_training)]

        for i, (button_text, callback) in enumerate(buttons):
            button_rect = pygame.Rect(sidebar_x + 10, y_offset + i * 35, 200, 30)
            mouse_pos = pygame.mouse.get_pos()
            button_color = (
                BUTTON_HOVER if button_rect.collidepoint(mouse_pos) else BUTTON_COLOR
            )
    ...
    def draw_menu(self):
        ...
        text = self.small_font.render(
            "Designed and developed by - MykeChidi", True, TEXT_COLOR
        )
    ...
    def handle_event(self, event):
        ...
        if event.type == pygame.QUIT:
            ...
            if event.button == 1:  # Left click
                ...
                if pos[0] < BOARD_SIZE and self.controller.game_state in [
                    GameState.HUMAN_VS_AI,
                    GameState.HUMAN_VS_HUMAN,
                    GameState.PUZZLE,
                ]:
