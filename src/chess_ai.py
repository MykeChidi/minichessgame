...
class ChessAI:
    ...
    def get_move(self, board: chess.Board, use_exploration: bool = True) -> chess.Move:
        ...
        if use_exploration and random.random() < self.epsilon:
            ...
            move_values = {
                move: self.q_table[state].get(move.uci(), 0.0) for move in legal_moves
            }
        ...
        if self.engine:
            try:
                ...
                return (
                    ...
                    else self._material_evaluation(board)
                )
            except Exception:
                pass
    ...
    def get_best_minimax_move(self, board: chess.Board) -> chess.Move:
        ...
        for move in board.legal_moves:
            ...
            move_value = self.minimax(
                board, self.minimax_depth - 1, maximizing_player=(not board.turn)
            )
            board.pop()
    ...
    def update_q_values(
        self, game_states: List[Tuple[str, chess.Move]], final_reward: float
    ):
        ...
        for i, (state, move) in enumerate(reversed(game_states)):
            ...
            momentum = 0.9
            self.q_table[state][move.uci()] = momentum * old_q + (1 - momentum) * (
    ...
    def train_single_game(self):
        ...
        while (
            not self.board.is_game_over() and len(game_states) < self.max_moves_per_game
        ):
